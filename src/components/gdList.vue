      ...
      <div class="item">工单状态
        ...
        <el-dropdown trigger="click">
          <div class="el-dropdown-link" >
            ...
            <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="Initstatus( status = -1 )"> 全 部 </el-dropdown-item>
              <el-dropdown-item @click="Initstatus( status = 0 )">未分配</el-dropdown-item>
              <el-dropdown-item @click="Initstatus( status = 1 )">未接受</el-dropdown-item>
              <el-dropdown-item @click="Initstatus( status = 2 )">进行中</el-dropdown-item>
              <el-dropdown-item @click="Initstatus( status = 3 )">已完成</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>  
      </div>
      <div class="item">接单人</div>
        ...
        <li class="ilList">
          <ul class="edit">
            <div class="edit-more">
              <div>
                <el-dropdown trigger="click">
                  <router-link to="gdChange">
                    <li class="edit-li" @click="ongdChange(item)">修改</li>
                  </router-link>
                </el-dropdown>
                <!-- 下拉选择分配 -->
                <el-dropdown trigger="click">
                  <div
                    ...
                    @click="onSearch(item.orderId, item.status)"
                  >
                    <li class="edit-li">分配</li>
                  </div>
                  <template #dropdown>
                ...
                <el-dropdown trigger="click">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item @click="onDelete(item.orderId)"
                        >确认</el-dropdown-item
                      >
              ...
              <div>
                <div class="el-dropdown-link">
                  <el-button type="primary" @click="toComponentTwo(item)"
                    >订单详情</el-button
                  >
                </div>
              </div>
...
export default {
  ...
  methods: {
    // 初始化数据
    async dataInit() {
      const res = await request("post", "/dispatch/order/getDivideOrderList", {
        status: -1,
        type: 0,
      });
    ...
    async onChangestate(item) {
      ...
      const res = await request(
        "put",
        "/dispatch/order/divide/" + orderId + "/" + Id
      );
      if (status != "未分配") {
        console.log(status);
        this.errorDiv();
    ...
    async onDelete(orderId) {
      const res = await request("delete", "/dispatch/order/remove", {
        orderId,
      });
